...
namespace LDGJ45.Editor
{
    public sealed class ImGuiRenderer
    {
        ...
        public ImGuiRenderer(GameWindow game, GraphicsDevice graphicsDevice)
        {
            var context = ImGui.CreateContext();
            ImGui.SetCurrentContext(context);

            _gameWindow = game ?? throw new ArgumentNullException(nameof(game));
        ...
        /// </summary>
        public IntPtr BindTexture(Texture2D texture) => ImGuiTextureBinder.GetOrCreateBinding(texture);
        ...
        private unsafe void RenderCommandLists(ImDrawDataPtr drawData)
        {
            ...
            for (int n = 0; n < drawData.CmdListsCount; n++)
            {
                ...
                for (int cmdi = 0; cmdi < cmdList.CmdBuffer.Size; cmdi++)
                {
                    ...
                    if (!ImGuiTextureBinder.ContainsKey(drawCmd.TextureId))
                    {
                    ...
                    var effect = UpdateEffect(ImGuiTextureBinder.GetTexture(drawCmd.TextureId));
